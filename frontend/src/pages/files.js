...
class FileManager extends Component {
    ...
    generateBreadcrumbs() {
        if (this.state.currentDirectory === "") return <li className="breadcrumb-item active" aria-current="page"><button type="button" className="btn btn-link" style={style.breadcrumb} disabled={true}>Home</button></li>;
        ...
        return paths.map((path, index) => <li key={index}
            ...
            <button type="button" className="btn btn-link" style={style.breadcrumb} disabled={index === paths.length - 1}>{ (index === 0) ? "Home" : path }</button>
        </li> );
    }
                    ...
                    <div className="col-sm-8">
                        <div className="card" style={{ height: "85vh" }}>
                            ...
                            <div className="card-body" style={{ overflow: "auto", paddingTop: "0px" }}>
                                ...
                                { !this.state.loadingFiles && this.state.children && this.state.children.map( data => (data.directory) ? <DirectoryEntry key={Math.random().toString()} directory={data}/> : <FileEntry key={Math.random().toString()} file={data}/>)}
                            </div>
                        </div>
                    </div>
                </div>
