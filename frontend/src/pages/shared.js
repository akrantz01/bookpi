      ...
      .then(data => {
        if (data.status !== 200 && data.status !== 401) toast.error(`Failed to load shared files: (${data.status}) ${data.reason}`)
          ...
          for (const share of data.data) {
            const [author, path] = share.split('/', 2)
            if (!Object.property.hasOwnProperty.call(shares, author)) shares[author] = [path]
              ...
              { !this.state.loading && Object.keys(this.state.shares).length !== 0 && Object.keys(this.state.shares).map(author => (
                <>
                  ...
                  { this.state.shares[author].map(file => (
                    <div className="card" style={{ marginTop: '0.25rem', marginBottom: '0.25rem' }} key={`${author}--${file}`}>
